    ...
    def monitor(self, ffmpeg, duration, time_, time_left, process):
        ...
        if self.print_monitor:
            ...
            sys.stdout.write(
                ...
                (per, datetime.timedelta(seconds=int(time_left)),
                 '#' * per, '-' * (100 - per))
            )
            sys.stdout.flush()
...
class ThreadManager():
    ...
    def new(self, file_path, id):
        ...
        if id not in self.poll:
            new_thread = encodeThread(
                file_path, self.work_path, name=file_name)
...
if __name__ == '__main__':
    ...
    inp = Path(
        "C:\\Users\\Guille\\Desktop\\downloads\\Los Simpsons\\Temporada 13\\a.mp4")
    n = ffmpegUtils.generate_thumbail(
        inp, Path("C:\\Users\Guille\\Desktop\\downloads\\Los Simpsons\\tes\\"))
    print(n)
